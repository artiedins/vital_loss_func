...
**almi_kg_m2** -- kg/m2
Appendicular Lean Mass Index from DEXA: (arms + legs lean mass in kg) / height in meters squared. Same DEXA scan as vat_cm2. Score 100 at >=8.7 kg/m2 (male) or >=7.5 kg/m2 (female), 0 at <=7.0 kg/m2 (male) or <=5.5 kg/m2 (female). Female poor threshold consistent with EWGSOP2 and Tromso Study sarcopenia cutoffs. Source: Cruz-Jentoft et al. Age Ageing 2019; Tromso Study 2015-16.
...
**vat_cm2** -- cm2
Visceral adipose tissue from DEXA (L1-L4 region). Same scan as ALMI. More metabolically relevant than total body fat. Score uses log scale: 100 at <=100 cm2, 0 at >=300 cm2.
...
2. Add them together (do NOT include DPA -- it adds no incremental predictive power to the Index per founding Harris/Von Schacky studies and the Atherosclerosis 2017 10-cohort meta-analysis)
3. Multiply the sum by 1.47 (whole-blood to RBC-membrane conversion, Stark et al. Prostaglandins 2016)
4. Enter that result as `omega3_1.47x_epa_dha_index_percent`

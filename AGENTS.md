...
- Fasting status matters for glucose and triglycerides -- always annotate.
...
# Draw date: 2025-03-01. Fasting: yes (12 hours).
...
**vo2_max_ml_kg_min** -- mL/kg/min (within-domain weight: 35%)
Maximum oxygen uptake. Must be from a proper incremental lab protocol (Bruce, Balke, or cycle ergometer equivalent) with a metabolic cart. Do not use formula estimates from resting HR or submaximal step tests -- these have +-15% error and will corrupt this domain. Confirm maximal effort via peak RER >= 1.10 before accepting the value. Score 100 at >=52 (male) or >=42 (female), 0 at \<=22 (male) or \<=19 (female). Sources: ACSM/Cooper Institute normative data; FRIEND database, Mayo Clinic Proceedings 2015.

**grip_strength_kg** -- kg (within-domain weight: 25%)
...
**heart_rate_recovery_bpm** -- bpm (within-domain weight: 20%)
...
**almi_kg_m2** -- kg/m2 (within-domain weight: 20%)
...
**resting_hr_bpm** -- bpm
Overnight minimum from a validated wearable that captures true cardiac minimum during sleep when sympathetic tone is lowest. Score 100 at \<=48 bpm, 0 at >=95 bpm.
...
**glucose_mg_dl** -- mg/dL (optional)
Fasting glucose. When present, blends with HbA1c (70/30) to form the glycemic component. If absent, HbA1c alone covers glycemic control. Always include if the blood draw was fasting. Score 100 at \<=85 mg/dL, optimal target 82 mg/dL (80-84 mg/dL is the T2D-risk nadir from a 12.4M-person cohort study).
...
GE Lunar machines report VAT in lbs. Convert before entering:
...
```yaml
# GE Lunar scan: VAT reported as 0.1 lbs
# 0.1 * 313.7 = 31.37
...
This domain requires a wearable for best accuracy. Missing keys fall back to defaults.

**sleep_regularity_index** -- 0-100 (within-domain weight: 40%)
Consistency of sleep/wake timing. Derivable from any wearable that tracks sleep/wake times over 7+ days. If the device does not report SRI directly, it can be computed as the average daily overlap of sleep windows across consecutive days. Use a 30-day rolling average. Score 100 at >=85, 0 at \<=45.

**sleep_duration_hours** -- hours (within-domain weight: 30%)
...
**hrv_ms** -- ms (RMSSD) (within-domain weight: 30%)
Root mean square of successive differences in R-R intervals. Two acceptable sources: (1) chest strap morning supine 5-minute protocol -- lie flat before rising, no caffeine; (2) wearable overnight average. The morning supine protocol produces a more controlled, reproducible number; overnight averages are convenient but blend readings across sleep stages. Do not use HRV readings from during exercise or from optical wrist sensors during activity. Note the source in a YAML comment. Score 100 at >=60 ms, 0 at \<=18 ms. **This value declines naturally with age even in healthy individuals -- a declining hrv_ms over years is expected biology, not failure.**

---

### Inflammation (domain weight: 12%)

**hs_crp_mg_l** -- mg/L (within-domain weight: 45%)
...
**homocysteine_umol_l** -- umol/L (within-domain weight: 30%)
...
**omega3_1.47x_epa_dha_index_percent** -- % RBC-membrane equivalent (within-domain weight: 25%)
The Omega-3 Index, expressed as RBC-membrane EPA+DHA equivalent. Many commercial omega-3 tests report whole-blood values; you must convert before entering.

**How to derive this value from a whole-blood EPA+DHA result:**
...
```yaml
# Whole-blood EPA+DHA: EPA 0.5, DHA 3.1, DPA 0.8 (DPA excluded)
# (0.5 + 3.1) * 1.47 = 5.3
...
More stable than creatinine; less confounded by muscle mass; strongest single blood predictor of mortality per Yang et al. 2023. Must be requested specifically -- often costs extra. Score uses log scale: 100 at \<=0.85 mg/L, 0 at >=2.0 mg/L.

**egfr_ml_min_1_73m2** -- mL/min/1.73m2 (within-domain weight: 25%)
Preferred: cystatin-C-based or combined CKD-EPI 2021 equation using both creatinine and cystatin C. Score 100 at >=90, 0 at \<=30.

**egfr_cr_ml_min_1_73m2** -- mL/min/1.73m2 (fallback input, not an independent scored slot)
Creatinine-only eGFR from standard blood panels. When `egfr_ml_min_1_73m2` is absent, the script silently promotes this value into the scoring slot. Note: in muscular individuals, creatinine-based eGFR has a known upward bias; the script flags this in output when the fallback is active. Enter this key from every standard panel -- it will be used automatically until a cystatin-C value is available.
...
1. **High-gradient DEFAULT keys** -- recommend testing. These are the keys most likely to move the loss.

1. **High-gradient measured keys** -- recommend interventions. Key levers by domain:

   - **fitness**: Zone 2 cardio (VO2 max, resting HR), resistance training (ALMI, grip strength, HRR)
   ...
   - **renal**: cystatin C and eGFR -- flag any concerning values for clinical evaluation; when the output note says "creatinine source; muscle-mass upward bias possible," recommend upgrading to a cystatin C panel; hydration and avoiding nephrotoxic substances are the primary lifestyle levers
...
When many keys are on defaults, the loss value has wide uncertainty. A loss of 0.15 with 18/22 keys measured is more reliable than 0.15 with 8/22. When advising, note how many keys are on defaults and which domains they fall in. The sleep domain is commonly all-defaults for users without a wearable -- flag this explicitly and note it affects the 12% sleep weight.
